...
// Middleware (não precisa recriar app aqui)
router.use(cors());
router.use(express.json());

// ✅ Rota de teste
router.get("/", (req, res) => {
  res.json({ status: "Ok - Paciente API" });
});
...
router.get("/getpaciente/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: true, message: "Erro interno ao buscar paciente" });
  }
});
...
router.post("/insertpaciente", async (req, res) => {
  try {
    ...
    res.status(201).json({ error: false, message: "Paciente inserido com sucesso", id: result.insertId });
  } catch (error) {
